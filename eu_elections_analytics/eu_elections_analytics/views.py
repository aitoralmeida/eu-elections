...
####################################################################################################
#####   View: hashtags_evolution()
####################################################################################################

def hashtags_evolution(request):
    result = []
    result.append([])
    result[0].append('Day')
    day_list = []
    ...
    try:
        ...
        cursor = con.cursor()
        cursor.execute("Select day from hash_group GROUP BY day ORDER BY day")
        node_t = cursor.fetchall()
        times = 1

        for nt in node_t:
            result.append([])
            result[times].append(str(nt[0]))
            day_list.append(str(nt[0]))
            times += 1

        cursor.execute("Select day, text, total from hash_group GROUP BY text ORDER BY total")
        ...
        for node in nodes[len(nodes)-5:]:
            ...
            for day in day_list:
                cursor.execute("Select day,text,total from hash_group WHERE text='%s'AND day = '%s' GROUP BY day" % (node[1], str(day)))
                node_t = cursor.fetchall()
    ...
    return render(request, "eu_elections_analytics/hashtags/evolution.html", return_dict)
...
####################################################################################################
#####   View: languages_evolution()
####################################################################################################

def languages_evolution(request):
    result = []
    result.append([])
    result[0].append('Day')
    day_list = []
    ...
    try:
        con = mysql.connector.connect(**config)
        cursor = con.cursor()
        cursor.execute("Select day from language_group GROUP BY day ORDER BY day")
        node_t = cursor.fetchall()
        times = 1
        ...
        cursor.execute("Select day, text, total from language_group GROUP BY text ORDER BY total")
        nodes = cursor.fetchall()
        ...
        for node in nodes[len(nodes)-5:]:
            ...
            for day in day_list:
                cursor.execute("Select day, text, total from language_group WHERE text='%s'AND day = '%s' GROUP BY day" % (node[1], str(day)))
                node_t = cursor.fetchall()
    ...
    return render(request, "eu_elections_analytics/languages/evolution.html", return_dict)
